...
export class VendaService {
  ...
  getPaginatedData(
    ...
    const hasSearchParams = searchParams && (searchParams.search?.trim() || searchParams.status);
  ...
  gerarNfe(vendaId: string, storeId: string): Observable<VendaResponseDto> {
    return this.http.post<VendaResponseDto>(`${this.apiUrl}/${vendaId}/gerar-nfe?storeId=${storeId}`, {});
  }

  create(data: VendaRequestDto): Observable<VendaResponseDto> {
    const storeId = this.storeContextService.currentStoreId;
    return this.http.post<VendaResponseDto>(`${this.apiUrl}?storeId=${storeId}`, data).pipe(
      tap((response) => {
  ...
  cancelVenda(vendaId: string, storeId: string): Observable<MessageResponse> {
    return this.http.delete<MessageResponse>(`${this.apiUrl}/${vendaId}?storeId=${storeId}`).pipe(
      tap((response) => {
